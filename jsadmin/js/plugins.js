var Plugins = function() {
    ...
    var m = function() {
        if ($.fn.dataTable) {
            ...
            $.fn.dataTable.defaults.aLengthMenu = [[10, 25, 50, -1], [ 10, 25, 50, "All"]];
            $(".datatable").each(function() {
                var r = $(this);
                var s = {};
                var v = r.data("datatable");
                if (typeof v != "undefined") {
                    $.extend(true, s, v)
                }
                var o = r.data("displayLength");
                if (typeof o != "undefined") {
                    $.extend(true, s, {iDisplayLength: o})
                }
                var q = r.data("horizontalWidth");
                if (typeof q != "undefined") {
                    $.extend(true, s, {sScrollX: "100%", sScrollXInner: q, bScrollCollapse: true})
                }
                if (r.hasClass("table-checkable")) {
                    $.extend(true, s, {aoColumnDefs: [{bSortable: false, aTargets: [0]}]})
                }
                if (r.hasClass("table-tabletools")) {
                    $.extend(true, s, {sDom: "<'row'<'dataTables_header clearfix'<'col-md-4'l><'col-md-8'Tf>r>>t<'row'<'dataTables_footer clearfix'<'col-md-6'i><'col-md-6'p>>>", oTableTools: {aButtons: ["copy", "print", "csv", "xls", "pdf"], sSwfPath: "plugins/datatables/tabletools/swf/copy_csv_xls_pdf.swf"}})
                }
                if (r.hasClass("table-colvis")) {
                    $.extend(true, s, {sDom: "<'row'<'dataTables_header clearfix'<'col-md-6'l><'col-md-6'Cf>r>>t<'row'<'dataTables_footer clearfix'<'col-md-6'i><'col-md-6'p>>>", oColVis: {buttonText: "Columns <i class='icon-angle-down'></i>", iOverlayFade: 0}})
                }
                if (r.hasClass("table-checkable") && r.hasClass("table-colvis")) {
                    $.extend(true, s, {oColVis: {aiExclude: [0]}})
                }
                if (r.hasClass("table-responsive")) {
                    var t;
                    var u = {tablet: 1024, phone: 480};
                    var p = $.fn.dataTable.defaults.fnDrawCallback;
                    $.extend(true, s, {bAutoWidth: false, fnPreDrawCallback: function() {
                            if (!t) {
                                t = new ResponsiveDatatablesHelper(this, u)
                            }
                        }, fnRowCallback: function(z, y, x, w) {
                            t.createExpandIcon(z)
                        }, fnDrawCallback: function(w) {
                            p.apply(this, w);
                            t.respond()
                        }})
                }
    ...
    var n = {type: "bar",
                height: "19px",
                zeroAxis: false, barWidth: "4px", barSpacing: "1px", barColor: "#fff"};
    var e = function() {
        if ($.fn.colorpicker) {
