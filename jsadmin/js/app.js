var App = function() {
    ...
    var v = "250px";
    var u = function() {
        var x = (navigator.userAgent.match(/msie [8]/i));
        var w = (navigator.userAgent.match(/msie [9]/i));
        var y = !!navigator.userAgent.match(/MSIE 10/);
        if (y) {
            $("html").addClass("ie10")
        }
    ...
    var r = function() {
        q();
        if ($.fn.dataTable) {
            var w = $.fn.dataTable.fnTables(true);
            $(w).each(function() {
    ...
    var f = function() {
        if ($(window).width() <= 767) {
            $("body").on("movestart", function(x) {
                ...
                var w = $(x.target).parents("#project-switcher");
                if (w.length) {
                    x.preventDefault()
                }
    ...
    var d = function() {
        var y = "icon-angle-down", x = "icon-angle-left";
        ...
        $("#sidebar-content ul > li > a").on("click", function(B) {
            ...
            if ($(window).width() > 767) {
                ...
                A.children("li.open").children("a").children("i.arrow").removeClass(y).addClass(x);
                A.children("li.open").children(".sub-menu").slideUp(200);
                A.children("li.open-default").children(".sub-menu").slideUp(200);
                A.children("li.open").removeClass("open").removeClass("open-default")
            }
            var z = $(this).next();
            if (z.is(":visible")) {
                $("i.arrow", $(this)).removeClass(y).addClass(x);
                $(this).parent().removeClass("open");
                z.slideUp(200, function() {
                    $(this).parent().removeClass("open-fixed").removeClass("open-default");
                    q()
                })
            } else {
                $("i.arrow", $(this)).removeClass(x).addClass(y);
                $(this).parent().addClass("open");
                z.slideDown(200, function() {
                    q()
                })
            }
            B.preventDefault()
                ...
                $(document).mousemove(function(C) {
                    ...
                    if (B <= 300 && B >= (z * 2 - 3)) {
                        if (B >= 240 && B <= 260) {
                            ...
                            $("#sidebar").css("width", B);
                            $("#sidebar-content").css("width", B);
                            $("#content").css("margin-left", B);
                            $("#divider").css("margin-left", B)
                        }
                    }
                })
            });
    ...
    var h = function() {
        var w = /android.*chrom(e|ium)/.test(navigator.userAgent.toLowerCase());
        if (/Android|webOS|iPhone|iPad|iPod|BlackBerry/i.test(navigator.userAgent) && w == false) {
            ...
            if ($("#sidebar").hasClass("sidebar-fixed") || $(window).width() <= 767) {
                ...
                $("#sidebar-content").slimscroll({height: "100%", wheelStep: x})
            }
        }
    };
    var e = function() {
    ...
    var i = function() {
        $(".widget .toolbar .widget-collapse").click(function() {
            var z = $(this).parents(".widget");
            var w = z.children(".widget-content");
            var y = z.children(".widget-chart");
            var x = z.children(".divider");
            if (z.hasClass("widget-closed")) {
                $(this).children("i").removeClass("icon-angle-up").addClass("icon-angle-down");
                w.slideDown(200, function() {
    ...
    var k = function() {
        $(".table-checkable thead th.checkbox-column :checkbox").on("change", function() {
            ...
            var w = $(this).parents("table.table-checkable").data("horizontalWidth");
            ...
            x.each(function(A, z) {
                $(z).find(".checkbox-column").each(function(C, B) {
                    var D = $(":checkbox", $(B)).prop("checked", y).trigger("change");
                    if (D.hasClass("uniform")) {
                        $.uniform.update(D)
                    }
        ...
        $(".table-checkable tbody tr td.checkbox-column :checkbox").on("change", function() {
            ...
            $(this).closest("tr").toggleClass("checked", w)
        })
    };
    var j = function() {
        var x = function(y) {
        ...
        x('.nav.nav-tabs.tabs-left > li.active > a[data-toggle="tab"], .nav.nav-tabs.tabs-right > li.active > a[data-toggle="tab"]');
        if (location.hash) {
    ...
    var p = function() {
        c();
        $(".project-switcher-btn").click(function(y) {
            y.preventDefault();
            w(this);
        ...
        $("body").click(function(z) {
            ...
            if ($.inArray("project-switcher", y) == -1 && $.inArray("project-switcher-btn", y) == -1 && $(z.target).parents().index($(".project-switcher")) == -1 && $(z.target).parents(".project-switcher-btn").length == 0) {
                w()
            }
        });
    ...
    return{init: function() {
            ...
            var w = $(w);
            w.block({message: '<img src="./assets/img/ajax-loading.gif" alt="">', centerY: x != undefined ? x : true, css: {top: "10%", border: "none", padding: "2px", backgroundColor: "none"}, overlayCSS: {backgroundColor: "#000", opacity: 0.05, cursor: "wait"}})
        }, unblockUI: function(w) {
            $(w).unblock({onUnblock: function() {
                    $(w).removeAttr("style")
                }})
        }}
}();